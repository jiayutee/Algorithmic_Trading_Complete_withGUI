...
class KuCoinConnector:
    ...
    There is no dedicated ``kucoin-python`` client in requirements.txt. The
    rest of this codebase already talks to KuCoin exclusively through
    ``ccxt.kucoin(...)`` (see ``core/data_loader.py::_get_kucoin_historical``
    and ``training_ground/TD3_trainer.py::fetch_kucoin_ohlcv``), so this
    connector follows the same convention rather than introducing a second
    KuCoin SDK.

    Parameters
    ----------
    api_key : str
        KuCoin API key. Ignored (may be empty string) when ``paper_mode=True``.
    secret_key : str
        KuCoin API secret. Ignored when ``paper_mode=True``.
    password : str
        KuCoin API passphrase (ccxt calls this ``password``). Ignored when
        ``paper_mode=True``.
    paper_mode : bool
        ...
        maintained — this mirrors ``BinanceConnector``'s ``paper_mode``
        exactly (same ledger shape, same ``place_order``/``get_portfolio``/
        ``get_position`` contract) so callers can treat both connectors
        interchangeably.

    Notes
    -----
    ``get_historical_klines`` fetches OHLCV candles from KuCoin's *public*
    market-data endpoint via ccxt, which requires no API credentials. Unlike
    ``BinanceConnector.get_historical_klines`` (which is gated behind
    ``paper_mode=False`` because python-binance's historical klines call uses
    an authenticated client), KuCoin historical data works in both paper and
    live mode here since the underlying ccxt call needs no keys.
    """
    ...
    def __init__(self, api_key: str = "", secret_key: str = "", password: str = "",
        ...
        self._public_client = ccxt.kucoin() if _CCXT_AVAILABLE else None
        ...
        if paper_mode:
            ...
            try:
                ...
                raise RuntimeError(f"Failed to initialise KuCoin client: {e}") from e

    # ------------------------------------------------------------------
    # Paper-trading helpers (mirrors BinanceConnector.place_order)
    # ------------------------------------------------------------------
    ...
    def place_order(self, symbol: str, side: str, qty: float, price: float) -> dict:
        ...
        ----------
        symbol : str
            Trading pair, e.g. ``"BTC/USDT"`` (ccxt/KuCoin unified symbol format).
        side : str
    ...
    def get_position(self, symbol):
        ...
        return {
            ...
            "qty": total,
            "side": "long",
            "avg_entry_price": None,  # KuCoin spot balances don't expose avg entry price
    ...
    def get_historical_klines(self, symbol: str, timeframe: str = "1d", since=None, limit: int = 100):
        """Fetch historical OHLCV candles from KuCoin's public market-data API.
        ...
        list
            ...
            returned by ``ccxt.kucoin().fetch_ohlcv``.
