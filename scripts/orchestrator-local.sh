...
" 2>/dev/null || echo "?")
    ...
    case "$RUN_TYPE" in
        morning)
            ...
            ;;
        work-loop)
            PROMPT="RUN_TYPE=work-loop. Work-loop cycle for Day ${DAY_N}/51 (catch-up run at ${LOCAL_TIME}). Follow the Work Loop procedure in .github/agents/orchestrator.agent.md exactly: STEP 0.5 foreign-dirty-tree guard first, then read today's Daily Log agenda, then use scripts/orchestrator_git.sh to create or reuse an owned task worktree inside .claude/worktrees, repeat pick/classify/execute/test-gate/commit/push-task-branch/open-PR/update-Notion for actionable items; never push main or auto-merge, keep pending-review tasks In progress, then send ONE consolidated Telegram summary at the end."
            ;;
        evening)
